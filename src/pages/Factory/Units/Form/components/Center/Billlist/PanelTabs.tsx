...
import { useListBillformTabs } from '../../../store/hooks';
...
const PanelTabs: FC<TPanelTabsProps> = ({ name }) => {
  ...
  const billFormTabs = useListBillformTabs(name);
    ...
    billFormTabs?.forEach((billformT) => {
      ...
      newTabPanes.push(
        <TabPane
          ...
          closable={true}
        >
          {name === EPartName.Header || name === EPartName.Body ? (
  ...
  const remove = (
    ...
    dispatch(
      actions.removeTableBillFormTab({
        name,
        idBillFormTab: targetKey as string,
      }),
    );
  };
  ...
  const handleOk = async () => {
    ...
    dispatch(actions.addTableBillFormTab({ name, billFormTab: values }));
  };
