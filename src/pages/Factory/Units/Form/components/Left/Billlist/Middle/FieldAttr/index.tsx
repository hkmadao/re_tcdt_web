...
import { TTip } from '@/pages/Factory/Units/Form/model';
  ...
  const refInputHandle = async (refConf?: TBillRef) => {
    dispatch(
      actions.updateTableBillFormField({
        ...
        dto: { ...tableBillFormField, refConfig: refConf },
      }),
    );
  };
  ...
  const enumInputHandle = async (enumConf?: TEnumRef) => {
    dispatch(
      actions.updateTableBillFormField({
        name: currentData?.name!,
        tabCode: currentData?.tabCode!,
        dto: { ...tableBillFormField, enumConfig: enumConf },
      }),
    );
  };
        ...
        <TabPane key={'base'} tab={'基础'} tabKey={'base'}>
          <Descriptions column={1} bordered size={'small'}>
            <Descriptions.Item label="ID">
              {tableBillFormField?.idBillFormField}
            </Descriptions.Item>
        ...
        <TabPane key={'advanced'} tab={'高级'} tabKey={'advanced'}>
          <Descriptions column={1} bordered size={'small'}>
            ...
            <Descriptions.Item label="引用设置">
              <RefConf
                value={tableBillFormField?.refConfig}
            ...
            <Descriptions.Item label="枚举设置">
              <EnumConf
                enumConfig={tableBillFormField?.enumConfig}
