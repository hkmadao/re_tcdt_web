...
const MainTableLayout: FC = () => {
  ...
  const onChange = (
    selectedRowKeys: Key[],
    selectedRows: TCommonAttribute[],
  ) => {};

  const handleSelect = (
    record: TCommonAttribute,
    select: boolean,
    selectedRows: TCommonAttribute[],
  ) => {
    if (rowSelectionType == 'checkbox') {
      const newKeys = selectedRows.map((r) => r.idCommonAttribute!);
      dispatch(actions.setSelectedRowKeys(newKeys));
    } else {
      dispatch(actions.setSelectedRowKeys([record.idCommonAttribute!]));
    }
  };
