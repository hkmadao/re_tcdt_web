...
/**渲染的图形 */
const MdEnumNodeItem: FC<TMdEnumItemProps> = (props: TMdEnumItemProps) => {
  ...
  const componentEnum = useSelector((state: { [x: string]: TModuleStore }) => {
    return state[moduleName].component.componentEnums?.find(
      (componentEnumFind) => componentEnumFind.idComponentEnum === idElement,
    );
  });
  ...
  useEffect(() => {
    if (componentEnum?.ddEnum?.attributes) {
      setDataSource(
        componentEnum?.ddEnum?.attributes.filter(
          (entityAttr) => entityAttr.action !== DOStatus.DELETED,
        ),
      );
    }
    const nodeUi = nodeUis?.find(
      (nodeUi) => nodeUi.idElement === componentEnum?.idComponentEnum,
    );
    if (nodeUi) {
        ...
        >
          <Row>
            ...
            <Col span={20} style={{ textAlign: 'center', fontSize: '16px' }}>
              &lt;&lt;枚举[{componentEnum?.ddEnum?.className}]&gt;&gt;
            </Col>
          </Row>
          <Row>
            <Col span={24} style={{ textAlign: 'center', fontSize: '16px' }}>
              {componentEnum?.ddEnum?.displayName}
            </Col>
          </Row>
