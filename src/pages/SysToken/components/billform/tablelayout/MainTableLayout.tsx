...
const MainTableLayout: FC = () => {
  ...
  const onChange = (selectedRowKeys: Key[], selectedRows: TToken[]) => {};
  ...
  const handleSelect = (
    ...
    if (rowSelectionType == 'checkbox') {
      const newKeys = selectedRows.map((r) => r.idSysToken!);
      dispatch(actions.setSelectedRowKeys(newKeys));
    } else {
      dispatch(actions.setSelectedRowKeys([record.idSysToken!]));
    }
  };
    ...
    return {
      onClick: (event: any) => {
        if (rowSelectionType == 'checkbox') {
          ...
          if (!newKeys.includes(record.idSysToken!)) {
            newKeys?.push(record.idSysToken!);
            dispatch(actions.setSelectedRowKeys(newKeys));
          } else {
            newKeys = newKeys?.filter((idKey) => idKey !== record.idSysToken);
            dispatch(actions.setSelectedRowKeys(newKeys));
          }
