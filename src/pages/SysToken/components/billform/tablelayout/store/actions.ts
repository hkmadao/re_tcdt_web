...
export const updateRecord: CaseReducer<TTableStore, PayloadAction<TToken>> = (
  state,
  action,
) => {
  if (state.tableData) {
    state.tableData = state.tableData.map((entity) => {
      if (entity.idSysToken === action.payload.idSysToken) {
        return { ...entity, ...action.payload };
      }
      return entity;
    });
  }
  const lastKey = action.payload.idSysToken;
  state.selectedRowKeys = [lastKey!];
};
