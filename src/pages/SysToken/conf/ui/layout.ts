import { TUiFactoryContent } from '@/models';

const layoutConf: TUiFactoryContent = {
  action: 1,
  componentModuleName: '系统模块',
  componentName: null,
  displayName: '令牌',
  fgTemplate: false,
  idComponent: '0m7OW2q0dEF7reUcjHqUN',
  idComponentModule: 'uFBamGsKfU3AwEHnqkH7E',
  idFactory: 'uU57iXGsBw2jRZ8aAS0nZ',
  idProject: 'HUo86fGpkXtIrqPKvjct0',
  idSubProject: '8JTsJB_8DgDd9eb__CfYT',
  name: 'SysToken',
  projectName: 'rtsp2rtmp',
  refIdContent: null,
  subProjectName: 'main',
  pages: [{ key: 'index', id: 'index', code: 'index', name: '首页' }],
  layouts: [
    {
      key: 'root',
      id: 'root',
      title: 'root',
      idParent: null,
      order: 1,
      type: 'layout',
      direction: 'column',
      flexType: 'auto',
      flexStr: '1 1 auto',
      children: [
        {
          key: 'H6FTe1dBekbr97pqRvzHM',
          id: 'H6FTe1dBekbr97pqRvzHM',
          title: '查询面板',
          idParent: 'root',
          order: 1,
          type: 'component',
          direction: 'row',
          flexType: 'notGrow',
          flexStr: '0 1 auto',
          children: [],
          component: {
            componentType: 'search',
            idRef: 'KnByfZoClTP2xp-rjihhG',
            name: '令牌',
          },
        },
        {
          key: 'CbgAlyjHO_SUxpmG3Ad5e',
          id: 'CbgAlyjHO_SUxpmG3Ad5e',
          title: '列表',
          idParent: 'root',
          order: 2,
          type: 'component',
          direction: 'row',
          flexType: 'auto',
          flexStr: '1 1 auto',
          children: [],
          component: {
            componentType: 'viewBillform',
            idRef: 'NHfdRpJHAn5SVzWt5svPq',
            name: '令牌',
          },
        },
        {
          key: 'oRt-4y7yx4N4XjBvmJJFF',
          id: 'oRt-4y7yx4N4XjBvmJJFF',
          title: '卡片按钮',
          idParent: 'root',
          order: 3,
          type: 'component',
          direction: 'row',
          flexType: 'notGrow',
          flexStr: '0 1 auto',
          children: [],
          component: { componentType: 'editButton' },
        },
        {
          key: '7Dwdniebj0nW5uIKoI1mc',
          id: '7Dwdniebj0nW5uIKoI1mc',
          title: '表单',
          idParent: 'root',
          order: 4,
          type: 'component',
          direction: 'row',
          flexType: 'auto',
          flexStr: '1 1 auto',
          children: [],
          component: { componentType: 'editBillform' },
          pageMaps: [
            {
              idPageMap: 'ba9901a6-6a83-4973-91c2-d8510800afe8',
              componentStateCode: 'list',
              pageCode: 'index',
            },
            {
              idPageMap: '2beb55f7-3edd-4932-9d26-9aeb0f48f838',
              componentStateCode: 'form',
              pageCode: 'edit',
            },
          ],
        },
      ],
    },
  ],
  assos: [
    { idPage: 'index', idLayout: 'root', hidden: false, disabled: false },
    {
      idPage: 'index',
      idLayout: 'H6FTe1dBekbr97pqRvzHM',
      hidden: false,
      disabled: false,
    },
    {
      idPage: 'index',
      idLayout: 'CbgAlyjHO_SUxpmG3Ad5e',
      hidden: false,
      disabled: false,
    },
    {
      idPage: 'index',
      idLayout: 'oRt-4y7yx4N4XjBvmJJFF',
      hidden: true,
      disabled: false,
    },
    {
      idPage: 'index',
      idLayout: '7Dwdniebj0nW5uIKoI1mc',
      hidden: true,
      disabled: false,
    },
  ],
};

export { layoutConf };
