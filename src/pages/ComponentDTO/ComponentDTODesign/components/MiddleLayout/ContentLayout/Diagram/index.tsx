...
const Diagrams: React.FC = () => {
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    // dispatch(
    //   actions.updateMouseCoordinates({ mouseX, mouseY, offsetX, offsetY }),
    // );
  };
  ...
  const handleMouseUp = (e: React.MouseEvent) => {
    const offsetX = engine.getModel().getOffsetX();
    const offsetY = engine.getModel().getOffsetY();
    dispatch(actions.updateOffSet({ offsetX, offsetY }));

    //按下右键
    if (e.button === 2) {
      if (addElementStatus) {
        dispatch(actions.setAddElementStatus({}));
      }
      if (connectionMode) {
        dispatch(actions.setConnectionMode({ connectionMode: false }));
      }
    }
    //左键按下
    if (e.button === 0) {
      ...
      //添加元素
      dispatch(actions.addElement({ mouseX, mouseY, offsetX, offsetY }));
    }
      ...
      .map((link) => {
        ...
        if (!link.getExtras()) {
          ...
          return {
            ...
            idDtoEntityCollection: entityCollection?.idDtoEntityCollection,
        ...
        >
          <CanvasWidget
            className={classnames({
              ...
              [styles.canvasWidgetMouse]: !!addElementStatus,
            })}
            engine={engine}
          />
        </div>
