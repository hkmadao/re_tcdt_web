...
const MdEnumSimpleNodeItem: FC<TMdEnumItemProps> = (
  ...
  const [dataSource, setDataSource] = useState<TDtoEnumAttribute[]>([]);
  ...
  const ddEnum = useSelector((state: { [x: string]: TModuleStore }) => {
    return state[moduleName].dtoCollection.dtoEnums?.find(
      (enumFind) => enumFind.idDtoEnum === idElement,
    );
  });
  ...
  useEffect(() => {
    if (ddEnum?.dtoEnumAttributes) {
      setDataSource(
        ddEnum?.dtoEnumAttributes.filter(
          (entityAttr) => entityAttr.action !== DOStatus.DELETED,
        ),
      );
    }
    const nodeUi = nodeUis?.find(
      (nodeUi) => nodeUi.idElement === ddEnum?.idDtoEnum,
    );
    if (nodeUi) {
  ...
  const columns = [
    {
      ...
      width: 40,
      render: (_text: any, record: TDtoEnumAttribute) => {
        let relation = '';
