...
export const fetchByTreeNode = createAsyncThunk(
  ...
  async (message: TMessage, thunkAPI) => {
    ...
    const pageInfo: TPageInfo<TDataType> = await ListAPI.pageList(params);
    return {
      selectedTreeNode,
      pageInfo,
    };
...
export const reflesh = createAsyncThunk(
  ...
  async (params: void, thunkAPI) => {
    ...
    const pageInfo: TPageInfo<TDataType> = await ListAPI.pageList(searchParam);
    return pageInfo;
  },
);
...
export const pageChange = createAsyncThunk(
  ...
  async (params: { page: number; pageSize: number }, thunkAPI) => {
    ...
    const pageInfo: TPageInfo<TDataType> = await ListAPI.pageList(queyrParams);
    return pageInfo;
  },
);
