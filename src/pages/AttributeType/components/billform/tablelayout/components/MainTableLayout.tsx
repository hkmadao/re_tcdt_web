...
const MainTableLayout: FC = () => {
  ...
  const onChange = (selectedRowKeys: Key[], selectedRows: TDataType[]) => {};
  ...
  const handleSelect = (
    ...
    if (rowSelectionType == 'checkbox') {
      const newKeys = selectedRows.map((r) => r.idDataType!);
      dispatch(actions.setSelectedRowKeys(newKeys));
    } else {
      dispatch(actions.setSelectedRowKeys([record.idDataType!]));
    }
  };

  const onRow = (record: TDataType) => {
    return {
      onClick: (event: any) => {
        if (rowSelectionType == 'checkbox') {
          let newKeys = tableStore.selectedRowKeys?.slice() || [];
          if (!newKeys.includes(record.idDataType!)) {
            newKeys?.push(record.idDataType!);
            dispatch(actions.setSelectedRowKeys(newKeys));
          } else {
            newKeys = newKeys?.filter((idKey) => idKey !== record.idDataType);
            dispatch(actions.setSelectedRowKeys(newKeys));
          }
