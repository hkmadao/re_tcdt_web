...
type ScanFilesResult = {
  pageDir: string;
  files: any[];
  dynamicRouters: TRoute[];
  dynamicMenus: MenuDataItem[];
};

function scanFiles(): ScanFilesResult {
...
function generateRoutes(
  ...
  `;
  fs.writeFile(dynamicRoutesFilePath, drs, function (err: any) {
    if (err) {
...
function generateMenus(
  ...
  `;
  fs.writeFile(dynamicMenusFilePath, dms, function (err: any) {
    if (err) {
      return console.error(err);
    }
...
function generateStores(
  ...
  `;
  fs.writeFile(dynamicStoreFilePath, dss, function (err: any) {
    if (err) {
      return console.error(err);
    }
...
function generate() {
  const dynamicRoutesFilePath: string = path.join(
    __dirname,
    'config/dynamic-routes.ts',
  );
  const dynamicMenusFilePath: string = path.join(
    __dirname,
    'src/menu/dynamic-menus.ts',
  );
  const dynamicStoreFilePath: string = path.join(
    __dirname,
    'src/store/dynamic-store.ts',
  );

  const scanResult = scanFiles();
  generateRoutes(dynamicRoutesFilePath, scanResult.dynamicRouters);
  generateMenus(dynamicMenusFilePath, scanResult.dynamicMenus);
  generateStores(dynamicStoreFilePath, scanResult.pageDir, scanResult.files);
}
